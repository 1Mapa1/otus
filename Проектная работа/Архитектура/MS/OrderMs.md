...
#### IdempotencyRecord
Техническая сущность для контроля идемпотентности у входящий запросов (для `POST /api/orders`).

Создание заказа и ответа в таблицу идемпотентности выполняются в одной транзакции.
### HTTP API
...
- `OrderRejectedV2 (`order.rejected.v2`)
