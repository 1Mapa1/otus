...
| CatalogMs   | Владелец информации о продуктах | Возвращает snapshot товаров и цен, так же проверяет правильность цен |
...
sequenceDiagram  
	...
	F->>G: POST /api/orders
	G->>OM: create order
	OM->>CM: POST /api/internal/catalog/products/snapshot
	note over CM: проверка цены<br/>(при расхождении PriceChanged 409)
	CM-->>OM: snapshot
